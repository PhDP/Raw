...
TEST_CASE("surrounded with xml node", "str") {
  REQUIRE(r_surrounded("<hello-world>", '<', '>'));
}
...
TEST_CASE("string concact with empty strings") {
  ...
  size_t length = 0;
  char* str;
  REQUIRE(0 == r_strcat(&str, &length, &capacity, ""));
}
...
TEST_CASE("string concact with empty destination") {
  ...
  size_t length = 0;
  char* str;
  REQUIRE(66 == r_strcat(&str, &length, &capacity, "All the world's a stage,\nAnd all the women and men merely players;"));
  REQUIRE(66 == length);
  REQUIRE(67 == capacity);
...
TEST_CASE("string concact with empty source") {
  ...
  REQUIRE(10 == r_strcat(&str, &length, &capacity, ""));
  REQUIRE(10 == length);
  REQUIRE(11 == capacity);
...
TEST_CASE("string concact with non-empty strings") {
  ...
  char* str = (char*)malloc(capacity);
  sprintf(str, "굴러온");
  REQUIRE(full_length == r_strcat(&str, &length, &capacity, " 돌이 박힌 돌 빼낸다"));
