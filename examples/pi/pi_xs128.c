...
int main(int argc, char **argv) {
  ...
  rd_xs128 r; // A random number generator.
  rd_xs128_init(&r, time(NULL)); // Initialize with the current time.
  ...
  for (; i < n; ++i) {
    ...
    const double x = rd_xs128_double(&r);
    const double y = rd_xs128_double(&r);
